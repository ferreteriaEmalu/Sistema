                ...
                firebase.auth().onAuthStateChanged((user) => {
                    try {
                        ...
                        for (let i = 0; i < rowObject.length; i++) {
                            ...
                            var PORCENTAJE = (VOLUMEN_GANANCIA / PRECIO_VENTA) * 100
                            var reservado = 0;
                            console.log(CODIGO,DESCRIPCION,PRECIO_COMPRA,STOCK,CATEGORIA,LIMITE_INFERIOR,proveedor,registradoPor,VOLUMEN_GANANCIA,PORCENTAJE,reservado)
                            
                            db.collection('productos').doc(CODIGO.toString()).set({
                                CODIGO,
                                DESCRIPCION,
                                PRECIO_COMPRA,
                                PRECIO_VENTA,
...
async function pintarTablaVenta(ventaGarray) {
        ...
        querySnapshot.forEach((doc) => {
            ...
            items.innerHTML += `
                <tr>
                    ...
                    <td>${datos.STOCK}</td>
                    <td><a id="${datos.CODIGO}" class="cursor" onclick="CambiarValorVenta(this)">${ventaGarray[i].valorProducto}</a></td>
                    <td><a id="${datos.CODIGO}" class="cursor" onclick="CambiarCantidadVenta(this)">${ventaGarray[i].cantidad}</a></td>
                    <th><a id="${datos.CODIGO}" class="cursor" onclick="EliminarItemVenta(this)"><img src="img/delete.png" width=20></a></th>
                </tr>
                `;
...
function CambiarValorVenta(element) {
    var valor;
    Swal.fire({
...
function CambiarCantidadVenta(element) {
    var cantidad_nueva;
    Swal.fire({
...
function EliminarItemVenta(element) {
